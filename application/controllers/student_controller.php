...
class Student_controller extends CI_Controller {
	...
	public function sms_marksheet() {
		...
		/*$class_id = 1;
		$exam_id = 4;
		$select_sql= $this->db->query("SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.ROLL_NO, SD.CLASS_ID, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID WHERE SD.STUDENT_DATA_ID NOT IN(
			SELECT STUDENT_DATA_ID FROM send_sms AS SS WHERE SS.CLASS_ID='".$class_id."' AND SS.EXAM_ID='".$exam_id."'
			)")->result();
		...
		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'sms_marksheet');
		$this->webspice->permission_verify('sms_marksheet');
		if( !isset($data['edit']) ){
			$data['edit'] = array(
				'CLASS_ID'=>null,
				'EXAM_ID'=>null,
		...
		$numbers = array();
		$student_id = $input->student_data_id;
		$class_id = $input->class_id;

		// dd($this->webspice->sms_string($class_id, $input->exam_id, $student_id));

		$data_query = $this->db->query("SELECT sd.*, si.* FROM student_data AS sd INNER JOIN student_info AS si ON si.STUDENT_ID=sd.STUDENT_ID WHERE sd.CLASS_ID='".$class_id."' AND sd.YEAR='".date("Y")."' AND si.PHONE <>'' AND sd.STUDENT_DATA_ID='".$student_id."'")->row();
		$number = (int)88 . $data_query->PHONE;
		// dd($number);

		$exam_data = $this->db->query("SELECT EXAM_NAME, EXAM_DATE FROM exam WHERE EXAM_ID='".$input->exam_id."'")->row();
		$exam_name = $exam_data->EXAM_NAME;
		$exam_date = $exam_data->EXAM_DATE;
		$exam_mark = $this->db->query("SELECT SUM(TOTAL_MARK) AS TOTAL_MARK FROM marks WHERE EXAM_ID='".$input->exam_id."' AND CLASS_ID='".$class_id."' AND STUDENT_ID='".$student_id."' GROUP BY EXAM_ID, CLASS_ID, STUDENT_ID")->row()->TOTAL_MARK;
		$total_examinee = $this->db->query("SELECT COUNT(*) AS COUNT_ALL FROM student_data WHERE CLASS_ID='".$class_id."'")->row()->COUNT_ALL;
		$obtained_mark = $this->db->query("SELECT SUM(MARK_OBTAINED) AS OBTAINED FROM marks WHERE CLASS_ID='".$class_id."' AND STUDENT_ID='".$student_id."' AND EXAM_ID='".$input->exam_id."' GROUP BY STUDENT_ID, EXAM_ID, CLASS_ID")->row()->OBTAINED;
		$highest_obtained = $this->db->query("SELECT MAX(H_OBTAINED) AS HIGHEST_MARK FROM (SELECT SUM(MARK_OBTAINED) AS H_OBTAINED FROM marks WHERE CLASS_ID='".$class_id."' AND EXAM_ID='".$input->exam_id."' GROUP BY STUDENT_ID, EXAM_ID, CLASS_ID) marks")->row()->HIGHEST_MARK;




		$position = $this->db->query("SELECT STUDENT_ID, SUM(MARK_OBTAINED) AS H_OBTAINED FROM marks WHERE CLASS_ID='".$class_id."' AND EXAM_ID='".$input->exam_id."' GROUP BY STUDENT_ID, EXAM_ID, CLASS_ID ORDER BY H_OBTAINED DESC")->result_array();

        for($i=0; $i<count($position); $i++) {
          array_push($position[$i], $i+1);
        }
        $papana = null;

        foreach($position as $pos_val) {
        	// dd($pos_val['STUDENT_ID'], true);
        	if($student_id == $pos_val['STUDENT_ID']) {
        		$papana = $pos_val[0];
        		// dd($pos_val[0], true);
        	}
        }
        ...
        $full_sms = $exam_name." Held on:".$exam_date."\n"."Total Mark:".$exam_mark." Total Examinee:".$total_examinee."\n"."Obtained Mark:".$obtained_mark." Position:".$papana."\n"."Highest Mark Obtained:".$highest_obtained."\n";
        // dd($full_sms);
		...
		foreach($mark as $mark_val) {
			$message .= $this->webspice->subject_name($mark_val->SUBJECT_ID) . ":(" . $mark_val->MARK_OBTAINED . "/" . $mark_val->TOTAL_MARK . ") ";
		}
		$message_val[] = $message;

		$full_sms .= $message_val[0];
		// dd($full_sms);
		...
		(CLASS_ID, EXAM_ID, STUDENT_DATA_ID, MESSAGE, CREATED_BY, CREATED_DATE, STATUS)
		VALUES
		( ?, ?, ?, ?, ?, ?, 7 )";
		$this->db->query($sql, array($class_id, $input->exam_id, $student_id, $full_sms, $this->webspice->get_user_id(), $this->webspice->now()));
		...
		var message = <?php echo json_encode($full_sms); ?>;
		var url_prefix = '/';
		...
		// for(var i=0; i<numbers.length; i++) {
			...
			setTimeout(function() {
				alert("SMS send successfully");
				<?php $this->webspice->message_board('SMS send successfully'); ?>
				window.location = url_prefix + 'sms_marksheet';
		...
		$this->webspice->force_redirect($url_prefix . 'sms_marksheet');*/

	}


	public function sms_classwise_result() {

		$url_prefix = $this->webspice->settings()->site_url_prefix;
		$this->webspice->user_verify($url_prefix.'login', $url_prefix.'sms_classwise_result');
		$this->webspice->permission_verify('sms_classwise_result');
		if( !isset($data['edit']) ){
			$data['edit'] = array(
				'CLASS_ID'=>null,
				'EXAM_ID'=>null,
		...
		if( !$this->form_validation->run() ){
			$this->load->view('admin/sms/sms_classwise_result', $data);
			return FALSE;
		}
		# get input post
		$input = $this->webspice->get_input('sms_id');
		...
		$data_query = $this->db->query("SELECT sd.STUDENT_DATA_ID, si.PHONE FROM student_data AS sd INNER JOIN student_info AS si ON si.STUDENT_ID=sd.STUDENT_ID WHERE sd.CLASS_ID='".$class_id."' AND sd.YEAR='".date("Y")."' AND si.PHONE <>''")->result();

		$numbers = array();
		$full_sms = array();
		for($i=0; $i<count($data_query); $i++) {
			$numbers[] = (int)88 . $data_query[$i]->PHONE;
			$exam_data = $this->db->query("SELECT EXAM_NAME, EXAM_DATE FROM exam WHERE EXAM_ID='".$exam_id."'")->row();
			$exam_name = $exam_data->EXAM_NAME;
			$exam_date = $exam_data->EXAM_DATE;
			$exam_mark = $this->db->query("SELECT SUM(TOTAL_MARK) AS TOTAL_MARK FROM marks WHERE EXAM_ID='".$exam_id."' AND CLASS_ID='".$class_id."' AND STUDENT_ID='".$data_query[0]->STUDENT_DATA_ID."' GROUP BY EXAM_ID, CLASS_ID, STUDENT_ID")->row()->TOTAL_MARK;
			$total_examinee = $this->db->query("SELECT COUNT(*) AS COUNT_ALL FROM student_data WHERE CLASS_ID='".$class_id."'")->row()->COUNT_ALL;
			$obtained_mark = $this->webspice->student_obtained_mark($class_id, $exam_id, $data_query[$i]->STUDENT_DATA_ID);
			$position = $this->webspice->student_merit_list($class_id, $exam_id, $data_query[$i]->STUDENT_DATA_ID);
			$highest_obtained = $this->webspice->exam_highest_mark($class_id, $exam_id);
			$init_string = $exam_name." Held on:".$exam_date."\n"."Total Mark:".$exam_mark." Total Examinee:".$total_examinee."\n"."Obtained Mark:".$obtained_mark." Position:".$position."\n"."Highest Mark Obtained:".$highest_obtained."\n";
			$mark_string = $this->webspice->sms_string($class_id, $exam_id, $data_query[$i]->STUDENT_DATA_ID);
			$full_sms[] = $init_string . $mark_string;
		}
		...
		VALUES
		( ?, ?, ?, ?, ?, ?, 7 )";
		for($j=0; $j<count($data_query); $j++) {
			$this->db->query($sql, array($class_id, $exam_id, $data_query[$j]->STUDENT_DATA_ID, $full_sms[$j], $this->webspice->get_user_id(), $this->webspice->now()));
		}
		...
		setTimeout(function() {
			alert("SMS send successfully");
			<?php $this->webspice->message_board('SMS send successfully'); ?>
			window.location = url_prefix + 'sms_classwise_result';
