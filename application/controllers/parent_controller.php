...
class Parent_controller extends CI_Controller {
	...
	**********************************************/
	...
	/*public function exam_wise_student_list() {

		$class_id = $this->input->post('class_id');
		$exam_id = $this->input->post('exam_id');

		$students_id = array();
		$students = $this->db->query("SELECT STUDENT_DATA_ID FROM send_sms WHERE CLASS_ID='".$class_id."' AND EXAM_ID='".$exam_id."'")->result();
		foreach($students as $s_val) {
			$students_id[] = $s_val->STUDENT_DATA_ID;
		}
		$students_id = implode(", ", $students_id);

		$select_sql= $this->db->query("SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.ROLL_NO, SD.CLASS_ID, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID WHERE SD.STUDENT_DATA_ID NOT IN(".$students_id.")")->result();

		$option_menu ='<option value="">--Select One--</option>';
		if(	$select_sql	)
		{
			foreach($select_sql as $list)
			{
	...
	public function exam_wise_student_list() {
		...
		$select_sql= $this->db->query("SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.ROLL_NO, SD.CLASS_ID, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID WHERE SD.STUDENT_DATA_ID NOT IN(".$students_id.") LIMIT 5")->result();*/
	...
	public function house_wise_student_list() {
		...
		$select_sql= "SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.CLASS_ID, SD.ROLL_NO, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID INNER JOIN admit_student_to_hostel AS ASTH ON ASTH.STUDENT_DATA_ID=SD.STUDENT_DATA_ID WHERE ASTH.HOUSE_ID='".$house_id."'";
	...
	public function section_wise_student_list() {
		...
		$select_sql= "SELECT SI.NAME, SD.STUDENT_DATA_ID, SD.CLASS_ID, SD.ROLL_NO, SD.STUDENT_ID FROM student_info AS SI INNER JOIN student_data AS SD ON SD.STUDENT_ID = SI.STUDENT_ID WHERE SD.SECTION_ID=? ORDER BY SD.ROLL_NO ASC";
