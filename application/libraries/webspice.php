...
class Webspice{
	...
	function settings(){
		...
		$settings->site_title = 'Base4 School Management';
		$settings->domain_name = 'Base4 School Management';
		$settings->site_slogan = '';
		$settings->site_url = 'http://shikdercadetacademy.com/';
		$settings->site_url_prefix = '/shikder/';
	...
	*/
	...
	public function mob_number_to_id($mob) {
		$CI = &get_instance();
		$CI->load->database();
		...
		$data = $CI->db->query("SELECT sd.*, si.* FROM student_data AS sd INNER JOIN student_info AS si ON si.STUDENT_ID=sd.STUDENT_ID WHERE si.PHONE='".$mob."'")->row()->STUDENT_DATA_ID;

		return $data;
	}

	public function roll_to_data_id($class_id, $roll) {
		/****************************************
		for another school use section field too
		****************************************/
		if(empty($roll)) {
			return false;
		}
		...
		$data_id = $CI->db->query("SELECT STUDENT_DATA_ID FROM student_data WHERE ROLL_NO='".$roll."' AND CLASS_ID='".$class_id."'")->row()->STUDENT_DATA_ID;
		return $data_id;

	}

	public function roll_to_section_id($class_id, $roll) {
		if(empty($roll)) {
			return false;
		}
		...
		$section_id = $CI->db->query("SELECT SECTION_ID FROM student_data WHERE ROLL_NO='".$roll."' AND CLASS_ID='".$class_id."'")->row()->SECTION_ID;
		return $section_id;

	}

	public function subject_name_to_id($class_id, $sub_name) {
		$CI = &get_instance();
		$CI->load->database();
		$subject_id = $CI->db->query("SELECT SUBJECT_ID FROM subject WHERE CLASS_ID='".$class_id."' AND SUBJECT_NAME='".$sub_name."'")->row()->SUBJECT_ID;
		return $subject_id;
	}

	public function student_merit_list($class_id, $exam_id, $student_id) {
		$CI = &get_instance();
		$CI->load->database();

		$position = $CI->db->query("SELECT STUDENT_ID, SUM(MARK_OBTAINED) AS H_OBTAINED FROM marks WHERE CLASS_ID='".$class_id."' AND EXAM_ID='".$exam_id."' GROUP BY STUDENT_ID, EXAM_ID, CLASS_ID ORDER BY H_OBTAINED DESC")->result_array();
	...
	public function student_obtained_mark($class_id, $exam_id, $student_id) {
		$CI = &get_instance();
		$CI->load->database();

		$obtained_mark = $CI->db->query("SELECT SUM(MARK_OBTAINED) AS OBTAINED FROM marks WHERE CLASS_ID='".$class_id."' AND STUDENT_ID='".$student_id."' AND EXAM_ID='".$exam_id."' GROUP BY STUDENT_ID, EXAM_ID, CLASS_ID")->row()->OBTAINED;
		return $obtained_mark;
	}

	public function exam_highest_mark($class_id, $exam_id) {
		$CI = &get_instance();
		$CI->load->database();

		$highest_obtained = $CI->db->query("SELECT MAX(H_OBTAINED) AS HIGHEST_MARK FROM (SELECT SUM(MARK_OBTAINED) AS H_OBTAINED FROM marks WHERE CLASS_ID='".$class_id."' AND EXAM_ID='".$exam_id."' GROUP BY STUDENT_ID, EXAM_ID, CLASS_ID) marks")->row()->HIGHEST_MARK;
		return $highest_obtained;

	}

	public function sms_string($class_id, $exam_id, $student_id) {
		$CI = &get_instance();
		$CI->load->database();

		$sms_string = "";
		$mark = $CI->db->query("SELECT * FROM marks WHERE CLASS_ID='".$class_id."' AND STUDENT_ID='".$student_id."' AND EXAM_ID='".$exam_id."' AND YEAR='".date("Y")."'")->result();

		$message = "";
		foreach($mark as $mark_val) {
			$message .= $CI->webspice->subject_name($mark_val->SUBJECT_ID) . ":(" . $mark_val->MARK_OBTAINED . "/" . $mark_val->TOTAL_MARK . ") ";
		}
		$message_val[] = $message;
		$sms_string .= $message_val[0];
